...
const CartPage = ({ closeOffcanvas }: { closeOffcanvas?: () => void }) => {
  ...
  const handleGoToPayment = () => {
    ...
    // 1. Đóng Offcanvas trước
    if (closeOffcanvas) {
      closeOffcanvas();
    }

    // 2. Chuyển trang (Sử dụng một chút delay để hiệu ứng đóng mượt hơn)
    setTimeout(() => {
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!isNaN(newQty) && newQty > 0) {
                            updateQuantity(item.id, newQty); // Giả sử context của bạn có hàm này
                          }
                        }}
                      />
                      <button
