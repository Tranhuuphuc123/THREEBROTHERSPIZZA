...
import {MaterialCreateTypes, ApiRespponseTypes} from '@/types/MaterialTypes'
...
import {MATERIAL_CREATE, MATERIAL_EDIT, MATERIAL_DELETE} from '@/constants/permissionsName'
...
export default function SupplierManage () {
  ...
  const [listMaterial, setListMaterial] = useState<MaterialCreateTypes[]>([]);
  ...
  */
  ...
  const canCreate = strPermission.includes(MATERIAL_CREATE);
  const canUpdate = strPermission.includes(MATERIAL_EDIT);
  const canDelete = strPermission.includes(MATERIAL_DELETE);
  ...
  const fetchAccounts = async () => {
    const res = await axiosAdmin.get<ApiRespponseTypes>('/materials', {
      params: {
