...
import {SalaryLevelTypes, ApiResponseTypes} from '@/types/SalaryLevelTypes'
...
import CreateModal from "@/app/admin/salary_level/create/page";
//import page form delete product
import DeleteModal from "@/app/admin/salary_level/delete/page";
//import form edit products
import UpdateModal from "@/app/admin/salary_level/edit/page";
...
import {SALARY_LEVEL_CREATE, SALARY_LEVEL_EDIT, SALARY_LEVEL_DELTE} from '@/constants/permissionsName'
...
export default function SalaryLevelManage () {
  ...
  const [listSalaryLevel, setListSalaryLevel] = useState<SalaryLevelTypes[]>([]);
  ...
  */
  ...
  const canCreate = strPermission.includes(SALARY_LEVEL_CREATE);
  const canUpdate = strPermission.includes(SALARY_LEVEL_EDIT);
  const canDelete = strPermission.includes(SALARY_LEVEL_DELTE);
  ...
  const fetchAccounts = async () => {
    const res = await axiosAdmin.get<ApiResponseTypes>('/salary_levels', {
      params: {
    ...
    if (res.data.data == null) {
      ...
      setListSalaryLevel(res.data.data); 
    }
      ...
      <div className="card p-3 manage-employees">
        <div className="row align-items-center mb-3 mx-1">
          <div className="col-12 col-sm-6 p-0 mb-2 mb-sm-0">
            <h5 className="ml-lg-2">Manage Salary Level</h5>
          </div>
