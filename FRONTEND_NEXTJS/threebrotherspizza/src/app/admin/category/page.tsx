...
import {CategoryTypes, ApiResponseTypes} from '@/types/CategoryTypes'
...
import CreateModal from "@/app/admin/category/create/page";
//import page form delete product
import DeleteModal from "@/app/admin/category/delete/page";
//import form edit products
import EditModal from "@/app/admin/category/edit/page";
...
import {CATEGORY_CREATE, CATEGORY_EDIT, CATEGORY_DELETE} from '@/constants/permissionsName'
...
export default function CategoryManage () {
  ...
  */
  ...
  const canCreate = strPermission.includes(CATEGORY_CREATE);
  const canUpdate = strPermission.includes(CATEGORY_EDIT);
  const canDelete = strPermission.includes(CATEGORY_DELETE);
  ...
  const fetchCategories = async () => {
    const res = await axiosAdmin.get<ApiResponseTypes>('/categories', {
      params: {
  ...
  return (
    ...
    <>
      ...
      <div className="card p-3 manage-employees">
        <div className="row align-items-center mb-3 mx-1">
          <div className="col-12 col-sm-6 p-0 mb-2 mb-sm-0">
            <h5 className="ml-lg-2">Manage Category</h5>
          </div>
      ...
      <Modal show={show && modalType === "edit"} onHide={closeModal}>
        <Modal.Header closeButton>
          <Modal.Title>Edit Category - edit value {selectedId}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedId !== null ? (
