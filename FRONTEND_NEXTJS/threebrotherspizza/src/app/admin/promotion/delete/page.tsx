...
import {DeleteProPropsTypes} from "@/types/PromotionTypes";

//import lib axios xử lý call api co mục select category và supplier id
import axiosAdmin from "@/axios/axiosAdmin";


const DeleteModal: React.FC<DeleteProPropsTypes> = ({
  id,
  onReload,
}) => {
  ...
  const handleDeleteClick = async () => {
      try {
        ...
        const response = await axiosAdmin.delete(`/promotions/delete/${id}`);

        // 2. Thông báo thành công
        showToast(response.data.msg || "Delete Promotion success!", "success");
