...
import {DeleteFeedbackPropsTypes, FeedbackTypes} from "@/types/FeedbackTypes";

//import lib axios xử lý call api co mục select category và supplier id
import axiosAdmin from "@/axios/axiosAdmin";


const DeleteModal: React.FC<DeleteFeedbackPropsTypes> = ({
  id,
  onReload,
}) => {
  ...
  const handleDeleteClick = async () => {
      try {
        ...
        const response = await axiosAdmin.delete(`/feedbacks/delete/${id}`);

        // 2. Thông báo thành công
        showToast(response.data.msg || "Delete review success!", "success");
  ...
  return (
    <>
      <p>Are you sure delete review ? {id}? </p>
